...
////////////////////////////////////////////////////////////////////////////////////////////////////////
  ...
  double chi2 = 1e10;
  double fit_value_tmp;
  for(double deltacp_test = 0; deltacp_test <= 360; deltacp_test += 90 ){
    for(double theta23_test = 40; theta23_test <= 50; theta23_test += 5 ){
      
    /* 1. 計算test value中 (delta_cp = NO) 的chi_N */
      glbDefineParams(test_values, theta12_N*degree, theta13_N*degree, theta23_test*degree,  deltacp_test*degree , 1e-5*sdm_g_N, 1e-3*ldm_g_N);  
    ...
    /* 2. 計算test value中 (delta_cp = IO) 的chi_I */
      glbDefineParams(test_values, theta12_I*degree, theta13_I*degree, theta23_test*degree,  deltacp_test*degree , 1e-5*sdm_g_I, 1e-3*ldm_g_I);  
      glbSetDensityParams(test_values,1.0,GLB_ALL); 
