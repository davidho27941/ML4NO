...
double degree    = M_PI/180;
 ...
 /* 定義global fit參數(Normal Ordering, NuFIT 5.0, 2020) */
  ...
  /* 定義chi2 Function */ 
double chi2 (int MO, double theta23,double deltacp)
{
  ...
  if(MO == -1){
  /* 定義true value (依照IO) */ 
    glbDefineParams(true_values,theta12_I*degree,theta13_I*degree,theta23_I*degree, 286*degree ,1e-5*sdm_g_I,1e-3*ldm_g_I);
    glbSetDensityParams(true_values,1.0,GLB_ALL);
  }
  ...
  /* 關閉系統誤差 */  
    glbSwitchSystematics(0,GLB_ALL,GLB_OFF);

  /* 計算chi2 */ 
    double chi_NO = glbChiNP(test_values_NO,NULL,0);
    double chi_IO = glbChiNP(test_values_IO,NULL,0);
  /* 取最小的chi2 */
...
{ 
  ...
  /* 建立輸出檔案 */
  FILE* OUT_DUNE_NO =    fopen("one_sigma_contour_DUNE_NO.dat","w");
  FILE* OUT_DUNE_IO =    fopen("one_sigma_contour_DUNE_IO.dat","w");
  ...
  /* DUNE */
  printf("DUNE_NO \n"); //看進度
  for(x=48.2;x<=49.5;x=x+0.01) // 推薦的theta23/deltacp設置範圍(1 sigma)
  for(y=178;y<=215;y=y+0.1)
  { 
    chi2_output = chi2(1, x ,y);
    printf("%g %g %g\n",x,y,chi2_output); //看進度

    if(chi2_output < 2.3 ) { 
    fprintf(OUT_DUNE_NO,"%g %g %g \n",x,y,chi2_output);
    }
  }

  /* DUNE */
  printf("DUNE_IO \n"); //看進度
  for(x=48.8;x<=50;x=x+0.01) // 推薦的theta23/deltacp設置範圍(1 sigma)
  for(y=260;y<=310;y=y+0.1)
  { 
    chi2_output = chi2(-1, x ,y);
    printf("%g %g %g\n",x,y,chi2_output); //看進度

    if(chi2_output < 2.3 ) { 
    fprintf(OUT_DUNE_IO,"%g %g %g \n",x,y,chi2_output);
    }
  }
