...
{ 
  ...
  glbDefineParams(true_values,theta12,theta13,theta23,deltacp,sdm,ldm);//定義true_values的值，暫時放旁邊(還沒真的設定)
  glbSetDensityParams(true_values,1.0,GLB_ALL);
  glbDefineParams(test_values,theta12,theta13,theta23,deltacp,sdm,ldm); //定義test_values的值，暫時放旁邊(還沒真的設定)
  glbSetDensityParams(test_values,1.0,GLB_ALL);

  /* The simulated data are computed */
  glbSetOscillationParameters(true_values);//設定true_values為OscillationParameters
  glbSetRates();
