...
/* 計算chi2 with projection onto theta23, 2 conditions for test value (NO) (IO)  */ 
//參數:{系統誤差on_off:[GLB_ON,GLB_OFF], 選定實驗EXP:[0,1,GLB_ALL], true value的theta23}
double chi2_proj (int on_off, int EXP , double theta23, double fit_values[10])
{
  ...
  /* 定義true value (依照NO) */ 
    glbDefineParams(true_values,theta12_N*degree,theta13_N*degree,theta23*degree, deltacp_N*degree ,1e-5*sdm_g_N,1e-3*ldm_g_N);
...
////////////////////////////////////////////////////////////////////////////////////////////////////////
  /* 1. 計算test value中 (delta_cp = NO) 的chi_N */
    glbDefineParams(test_values, theta12_N*degree, theta13_N*degree, theta23*degree,  deltacp_N*degree , 1e-5*sdm_g_N, 1e-3*ldm_g_N);  
  ...
  /* 3. 計算test value中 (delta_cp = IO) 的chi_I */
    glbDefineParams(test_values, theta12_I*degree, theta13_I*degree, theta23*degree,  deltacp_N*degree , 1e-5*sdm_g_I, 1e-3*ldm_g_I);  
    glbSetDensityParams(test_values,1.0,GLB_ALL); 
...
if (atof(argv[1]) == 0){
    filename = "best_fit_spectrum_theta23_DUNE.dat";//建立輸出檔案
}
else if (atof(argv[1]) == 1){
    filename = "best_fit_spectrum_theta23_T2HK.dat";//建立輸出檔案
}
else if (atof(argv[1]) == -1){
    filename = "best_fit_spectrum_theta23_DUT2.dat";//建立輸出檔案
}
...
FILE* OUT =  fopen(filename,"w");
  ...
  double a0 = chi2_proj(GLB_OFF, atof(argv[1]),  theta23, fit_values);   // 0代表DUNE, 1代表T2HK
  
  fprintf(OUT,"%g %g %g %g %g %g %g %g %g %g %g %g %g ",theta12_N*degree,theta13_N*degree,theta23*degree, deltacp_N ,1e-5*sdm_g_N,1e-3*ldm_g_N,
